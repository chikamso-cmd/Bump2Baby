...
export const WelcomeStep = ({ onNext }) => (
  <StepCard>
    ...
    <p className="text-gray-500 text-center text-sm mb-10 leading-relaxed px-4">
      Bump2baby is your trusted companion through pregnancy and early
      parenthood. Get personalized guidance, track milestones, and connect with
  ...
  const roles = [
    {
      id: "pregnant",
      title: "Pregnant",
      desc: "Expecting and tracking your pregnancy",
      icon: "🤰",
    },
    {
      id: "new-parent",
      title: "New Parent",
      desc: "Raising your newborn or young child",
      icon: "👶",
    },
    {
      id: "caregiver",
      title: "Caregiver",
      desc: "Supporting a loved one through their journey",
      icon: "👪",
    },
  ];
        ...
        {roles.map((role) => (
          <button
            ...
            className={`w-full flex items-center p-4 rounded-2xl border transition-all text-left ${
              selected === role.id
                ? "border-[#D83D6C] bg-pink-50 ring-1 ring-[#D83D6C]"
            ...
            <div>
              <p
                className={`font-semibold text-sm ${
                  selected === role.id ? "text-[#D83D6C]" : "text-gray-800"
                }`}
              >
                {role.title}
              </p>
...
export const BabyAgeStep = ({ selected, onSelect, onNext }) => {
  ...
  return (
    <StepCard>
      ...
      <p className="text-gray-400 text-xs mb-8">
        This helps us provide personalized content
      </p>
      <div className="w-full">
        <div className="grid grid-cols-3 gap-3 mb-6">
          {ages.slice(0, 9).map((age) => (
            <button
              ...
              className={`py-3 px-2 text-[11px] rounded-xl border transition-all font-medium ${
                selected === age
                  ? "bg-[#E6F4FF] border-[#66B5F0] text-[#007AFF]"
        ...
        <button
          onClick={onNext}
          disabled={!selected}
          className="w-full bg-[#D83D6C] text-white py-4 rounded-full font-semibold hover:bg-[#c1325d] transition-colors"
        >
          Continue
        </button>
      </div>
...
export const AccountStep = ({ onNext, setUserName, setHandle, role }) => {
  ...
  return (
    <StepCard>
      ...
      {error && (
        <div className="w-full bg-red-50 text-red-500 text-xs p-3 rounded-xl mb-4 text-center">
          {error}
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="w-full space-y-4">
        ...
        <button
          type="submit"
          disabled={isLoading}
          className="w-full bg-[#D83D6C] text-white py-4 rounded-full font-semibold hover:bg-[#c1325d] transition-colors mt-2 flex items-center justify-center disabled:opacity-70 disabled:cursor-not-allowed"
        >
          {isLoading ? (
            <>
...
export const AccountReadyStep = ({ onNext }) => (
  <StepCard>
    <div className="flex flex-col items-center justify-center py-10">
      ...
      <button
        onClick={onNext}
        className="w-full bg-[#D83D6C] text-white py-4 rounded-full font-semibold px-20 hover:bg-[#c1325d] transition-colors"
      >
        Continue
      </button>
    </div>
  ...
  return (
    <StepCard>
      ...
      <div
        ...
        className={`w-14 h-7 rounded-full relative transition-all cursor-pointer mb-8 ${
          notifs ? "bg-[#D83D6C]" : "bg-gray-200"
        }`}
      >
        <div
          className={`absolute top-1 w-5 h-5 rounded-full bg-white transition-all ${
            notifs ? "left-8" : "left-1"
          }`}
        />
      </div>
      <button
  ...
  return (
    <div className="flex flex-col items-center justify-center p-8 text-center animate-in fade-in zoom-in duration-500">
      <div className="bg-pink-50 p-6 rounded-full mb-6 relative">
        ...
        <svg
          className="w-12 h-12 text-[#D83D6C] relative z-10"
          fill="currentColor"
          viewBox="0 0 24 24"
        >
          <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z" />
